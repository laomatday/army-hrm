...
async function showMainApp() {
  ...
  await Promise.all([checkNewNotifications(), loadHistoryFull(), loadMyRequests(), loadLocations()]);
}
  ...
  container.innerHTML =
    data?.length > 0
      ? data
          .map(
            (req) =>
              `<div class="bg-white p-5 rounded-[24px] shadow-sm border border-white mb-4 animate-slide-up"><div class="flex justify-between items-start mb-4"><div class="flex gap-4"><div class="w-11 h-11 rounded-2xl bg-slate-50 flex items-center justify-center text-emerald-600"><i class="fa-solid fa-file-lines"></i></div><div><h4 class="font-black text-slate-800 text-sm">${req.Type}</h4><p class="text-[10px] text-slate-400">${req["From Date"]} - ${req["To Date"]}</p></div></div><span class="badge ${req.Status === "Approved" ? "badge-success" : "badge-info"}">${req.Status}</span></div><p class="text-xs text-slate-500 italic bg-slate-50 p-3 rounded-2xl">"${req.Reason}"</p></div>`
          )
          .join("")
      : `<p class="text-center py-10">Chưa có đề xuất.</p>`;
};
...
function switchActivityMode(m) {
  ...
  document.getElementById("btn-tab-requests").className =
    m === "requests"
      ? "px-4 py-1.5 rounded-full text-[10px] font-bold bg-emerald-600 text-white shadow-md transition-all"
      : "px-3 py-1.5 rounded-full text-[10px] font-bold text-slate-500 hover:text-slate-800 transition-all";
  document.getElementById("view-act-history").classList.toggle("hidden", m !== "history");
  document.getElementById("view-act-requests").classList.toggle("hidden", m !== "requests");
}
function handleConfirmReject() {
  var reason = document.getElementById("input-reject-reason").value;
  if (!reason) return showToast("error", "Nhập lý do!");
