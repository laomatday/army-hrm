...
window.handleLogin = function() {
    ...
    google.script.run
        ...
        .doLogin(emailEl.value.trim(), passEl.value.trim(), myDeviceId);
};
...
function loadHistoryFull() {
    ...
    google.script.run
        ...
        .getHistory(currentUser.Employee_ID);
}
...
window.openNotifications = function(mode) {
    ...
    google.script.run
        .withSuccessHandler(function(res) {
            var hasApprovals = res.data.approvals && res.data.approvals.length > 0;
            var hasMyRequests = res.data.myRequests && res.data.myRequests.length > 0;
...
window.processRequestMobile = function(reqId, status, rejectReason) {
    ...
    google.script.run
        ...
        .processRequestAdmin(reqId, status, note, currentUser.Name);
};
...
function loadContacts() {
    ...
    google.script.run
        ...
        .getContacts(currentUser.Role, currentUser.Center_ID);
}
...
function loadMyRequests() {
    ...
    google.script.run
        ...
        .getMyRequests(currentUser.Employee_ID);
}
...
function updateClock() {
    var d = new Date();
    var timeStr = d.toLocaleTimeString("vi-VN", {
        hour: "2-digit",
        minute: "2-digit",
        hour12: false
    });
