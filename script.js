 ...
 * Đã tối ưu hóa, sửa lỗi ReferenceError và tương thích 100% với Backend
 * ====================================================================
 */

// [QUAN TRỌNG] URL Web App từ Google Apps Script
...
// ==========================================
// 1. CÁC HÀM TIỆN ÍCH (UTILS) - ĐƯA LÊN ĐẦU ĐỂ TRÁNH LỖI
// ==========================================
...
function showMainApp() {
  ...
  // GỌI SUPER API (Đã có trong code.js của bạn)
  loadDashboardData(); 
}
...
async function loadDashboardData() {
  ...
  if (res.success) {
      ...
      // 3. XỬ LÝ THÔNG BÁO
      // Lưu ý: d.notifications là object { success: true, data: {...}, role: ... }
      // Ta cần truyền d.notifications.data vào renderNotificationsBadge
      if (d.notifications && d.notifications.success) {
          const notiPayload = d.notifications.data;
          cachedNotifications = notiPayload;
          renderNotificationsBadge(notiPayload);
          
          // Render nếu đang mở modal
          if (!document.getElementById("modal-notifications").classList.contains("hidden")) {
              ...
              renderNotificationContent(notiPayload, mode);
          }
      }
      ...
      if (d.myRequests) {
          ...
          renderMyRequestsHTML(document.getElementById("request-list-container"), d.myRequests);
      }
...
async function loadContacts() {
  ...
  if (!list) return;
  // Chỉ hiện skeleton nếu chưa có data
  if(!cachedContacts || cachedContacts.length === 0) list.innerHTML = SKELETON_CONTACT;
  
  // Dùng Super API để load, không gọi lẻ
  // Nếu muốn gọi lẻ: callBackend("getContacts", ...)
}
...
// ... Các logic Checkin/Camera/Submit Request/Profile Update giữ nguyên như bạn đã viết ...
// Tôi sẽ đảm bảo chúng nằm ở đây
