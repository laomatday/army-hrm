...
var cachedContacts = [];
var cachedLocations = [];
var cachedMyRequests = null; // Biến cache cho danh sách đề xuất
...
async function showMainApp() {
  ...
  await Promise.all([
    ...
    loadHistoryFull(), 
    loadMyRequests(true), // Load ép buộc lần đầu
    loadLocations()
  ]);
}
...
window.switchTab = function (tabName) {
  ...
  else if (tabName === "requests") switchActivityMode('requests');
};
...
// ==========================================
// 6. TẠO ĐỀ XUẤT (FIX LỖI & CACHE)
// ==========================================

// [NEW] Hàm vẽ giao diện Đề xuất (Tách ra để dùng chung)
...
window.loadMyRequests = async function(forceReload = false) {
  ...
  // 2. Hiện Skeleton nếu phải tải lại
  ...
  // 4. Kiểm tra Mảng trước khi render
...
window.submitRequest = async function () {
  ...
  if (r.success) { 
      ...
      closeRequestModal(); 
      cachedMyRequests = null; // Xóa cache
      loadMyRequests(true); // Tải lại
  }
};
...
function switchActivityMode(mode) {
    ...
    if (mode === "requests") {
        // [QUAN TRỌNG] Sử dụng CACHE khi chuyển tab
