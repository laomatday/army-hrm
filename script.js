 ...
 * Đã chuyển đổi từ code gốc, giữ nguyên logic Super API & UI
 * ====================================================================
 */

// [QUAN TRỌNG] Thay URL Web App của bạn vào đây
const API_URL = "https://script.google.com/macros/s/AKfycbxF_o_YAbBL1VIdf7HhRLT9WX9Gow7CG96tWEQtVq7LDisP0jj34esh5ZJ3uMuVaXs_/exec";
...
document.addEventListener("DOMContentLoaded", function () {
    // Khôi phục trạng thái chấm đỏ ngay lập tức (nếu có logic này)
...
window.handleLogin = async function () {
    ...
    showLoading(true);
    // Thay thế google.script.run bằng callBackend
...
async function loadDashboardData() {
    ...
    if (res.success) {
        ...
        // 1. CẬP NHẬT PROFILE (Quan trọng để đồng bộ Avatar/Số phép/Chức vụ)
        if (d.userProfile) {
        ...
        if (d.notifications) {
            cachedNotifications = d.notifications; // Lưu vào biến toàn cục (Cache)
            renderNotificationsBadge(d.notifications); // Chỉ hiện chấm đỏ

            // (Tùy chọn) Nếu Modal thông báo đang mở thì render lại nội dung luôn
        ...
        // 5. CACHE ĐỊA ĐIỂM (Cho Dropdown Profile)
        cachedLocations = d.locations || [];

        // 6. CACHE & RENDER DANH BẠ (Render ngầm để chuyển tab mượt mà)
        if (d.contacts) {
...
function renderHistoryStats(summary) {
    ...
    // 1. Tính toán ngày công chuẩn (Client side approximation)
...
function renderNotificationsBadge(notiData) {
    ...
    // 2. Đếm đơn của tôi có kết quả mới (Cho NV) -> Dựa vào LocalStorage
    var myUnreadCount = 0;
...
function renderMyRequestsList(requests) {
    ...
    requests.forEach(function (req) {
        ...
        // Logic check unread badge cho đơn của tôi
...
window.openNotifications = async function (mode) {
    ...
    // Ẩn chấm đỏ ngay lập tức
    document.querySelectorAll('[id^="noti-dot"], [id^="profile-noti-dot"]').forEach(d => d.classList.add("hidden"));
    modal.classList.remove("hidden");

    // CHIẾN THUẬT: HIỂN THỊ CACHE NGAY LẬP TỨC
    if (cachedNotifications) {
        renderNotificationContent(cachedNotifications, mode);
        markNotificationsAsRead(cachedNotifications); // Đánh dấu đã đọc
    } else {
    ...
    // Gọi ngầm Server để đồng bộ dữ liệu mới nhất
...
// Bổ sung các hàm helper thiếu để tránh lỗi ReferenceError
