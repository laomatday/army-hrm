...
// ==========================================
const API_URL =
  "https://script.google.com/macros/s/AKfycbxbuZVUPQ3W8QB-qrOxz0Y2GRMYbhJyrTtALToLn3fnz_HUDtDv5k-XZyKgjJj1qmhuKA/exec";

var currentUser = null;
var videoStream = null;

// --- CÁC BIẾN CACHE (BỘ NHỚ ĐỆM) ---
// Dữ liệu sẽ nằm ở đây sau lần tải đầu tiên, giúp chuyển tab siêu tốc
var allHistoryData = [];
var cachedContacts = [];
var cachedLocations = [];
...
const SKELETON_CONTACT =
  `<div class="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-3 mb-3 animate-pulse"><div class="w-12 h-12 rounded-2xl bg-slate-200"></div><div class="flex-1 space-y-2"><div class="h-4 w-32 bg-slate-200 rounded-full"></div><div class="h-3 w-20 bg-slate-200 rounded-full"></div></div><div class="w-10 h-10 rounded-2xl bg-slate-200"></div></div>`.repeat(
    5
  );
const SKELETON_REQUEST =
  `<div class="bg-white p-5 rounded-[24px] shadow-sm border border-white mb-4 animate-pulse"><div class="flex justify-between items-start mb-4"><div class="flex gap-4"><div class="w-11 h-11 rounded-2xl bg-slate-200"></div><div class="space-y-2"><div class="h-4 w-24 bg-slate-200 rounded-full"></div><div class="h-3 w-16 bg-slate-200 rounded-full"></div></div></div><div class="w-16 h-6 bg-slate-200 rounded-lg"></div></div><div class="h-10 w-full bg-slate-200 rounded-2xl"></div></div>`.repeat(
    3
  );
...
async function showMainApp() {
  ...
  // Tải song song tất cả dữ liệu nền để sẵn sàng cho việc chuyển tab
  await Promise.all([
    checkNewNotifications(),
    loadHistoryFull(),
    loadMyRequests(true), // Load lần đầu ép buộc lấy mới
    loadLocations(),
  ]);
}
...
window.switchTab = function (tabName) {
  ["modal-notifications", "modal-request", "modal-profile", "modal-contact-detail", "modal-search-contact"].forEach(
    (id) => {
  ...
  // Load Lazy (Chỉ khi chưa có cache mới tải)
...
function renderUserInfo() {
  ...
  document
    .getElementById("btn-profile-approval")
    ?.classList.toggle("hidden", adminRoles.indexOf(currentUser.Role) === -1);
  ...
  document
    .querySelectorAll("#user-avatar, #profile-user-avatar, #edit-avatar-preview")
...
window.takePicture = function () {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    () => {
      showLoading(false);
      showDialog("error", "Lỗi GPS", "Vui lòng bật định vị!");
    },
    { timeout: 10000 }
  );
};
...
// ==========================================
// 6. TẠO ĐỀ XUẤT (CÓ CACHE)
// ==========================================

// [OPTIMIZED] Hàm tải đề xuất với Cache thông minh
window.loadMyRequests = async function(forceReload = false) {
  var container = document.getElementById("request-list-container");
  if (!container) return;

  // 1. Nếu đã có Cache và không bị ép tải lại -> Dùng luôn (SIÊU NHANH)
  ...
  // 4. [QUAN TRỌNG] Kiểm tra dữ liệu trả về có phải là Mảng (Array) không
  if (Array.isArray(data)) {
      cachedMyRequests = data;
      renderMyRequestsHTML(cachedMyRequests);
  } else {
      // Nếu Server trả về lỗi (Object) thay vì danh sách
      console.error("Lỗi tải đề xuất:", data);
      
      // Hiển thị thông báo lỗi nhẹ nhàng thay vì crash ứng dụng
      container.innerHTML = `
        <div class="text-center py-12 opacity-60">
            ...
            <p class="text-[10px] text-slate-400 mt-1">${data.message || "Lỗi kết nối Server"}</p>
            <button onclick="loadMyRequests(true)" class="mt-3 px-4 py-2 bg-white border border-slate-200 rounded-full text-[10px] font-bold shadow-sm active:scale-95">Thử lại</button>
        </div>`;
...
// Hàm tách riêng để vẽ giao diện (Dùng chung cho cả Cache và Fetch mới)
...
window.submitRequest = async function () {
  ...
  if (r.success) {
    showToast("success", r.message);
    closeRequestModal();
    // XÓA CACHE ĐỂ ÉP TẢI LẠI DỮ LIỆU MỚI
...
window.submitProfileUpdate = async function () {
  ...
  if (res.success) {
    ...
    currentUser.Center_ID = currentProfileLocation;
    localStorage.setItem("army_user_v2026", JSON.stringify(currentUser));
    renderUserInfo();
    closeProfileModal();
  }
...
function renderLocationList() {
  ...
  if (list)
    list.innerHTML = cachedLocations
      .map(
        (loc) =>
          `<div onclick="selectProfileLocation('${loc.id}', '${loc.name}')" class="px-4 py-3.5 rounded-xl text-sm font-bold text-slate-700 hover:bg-slate-50 cursor-pointer mb-1 flex justify-between"><span>${loc.name}</span><i class="fa-solid fa-check text-emerald-500 opacity-0"></i></div>`
      )
      .join("");
}
...
// ==========================================
// Cập nhật hàm switchActivityMode để sử dụng Cache
function switchActivityMode(mode) {
  document.getElementById("btn-tab-history").className =
    mode === "history"
      ? "px-4 py-1.5 rounded-full text-[10px] font-bold bg-emerald-600 text-white shadow-md transition-all"
      : "px-3 py-1.5 rounded-full text-[10px] font-bold text-slate-500 hover:text-slate-800 transition-all";
  document.getElementById("btn-tab-requests").className =
    mode === "requests"
      ? "px-4 py-1.5 rounded-full text-[10px] font-bold bg-emerald-600 text-white shadow-md transition-all"
      : "px-3 py-1.5 rounded-full text-[10px] font-bold text-slate-500 hover:text-slate-800 transition-all";
  ...
  if (mode === "requests") {
    // [QUAN TRỌNG] Gọi loadMyRequests không ép buộc tải lại -> Sẽ dùng Cache
    loadMyRequests(false);
  } else {
    // Lịch sử đã có sẵn trong biến allHistoryData, render ngay lập tức
    renderActivityHistory();
  }
}
...
window.showDialog = (t, tl, m, cb) => {
  ...
  setText("dialog-title", tl);
  setText("dialog-msg", m);
  document.getElementById("dialog-icon").className =
    t === "error"
      ? "fa-solid fa-circle-exclamation text-4xl text-red-500"
      : "fa-solid fa-circle-check text-4xl text-emerald-600";
...
window.showToast = (type, m) => {
  ...
  setText("toast-msg", m);
  x.querySelector("div").className =
    `w-8 h-8 rounded-full flex items-center justify-center shadow-sm ${type === "error" ? "bg-red-100 text-red-600" : "bg-emerald-100 text-emerald-600"}`;
...
window.openExplainModal = (d, err) => {
  openRequestModal("Giải trình", d);
  var input = document.getElementById("req-reason");
  if (input) {
    input.value = `[${err}] `;
    input.focus();
  }
};
window.openRequestModal = (type, date) => {
  toggleGlobalNav(false);
  document.getElementById("modal-request").classList.remove("hidden");
  selectReqType(type || "Nghỉ phép");
  if (date) {
    var d = toISODate(date);
    document.getElementById("req-date-start").value = d;
    document.getElementById("req-date-end").value = d;
  }
};
...
window.openContactByIndex = (i) => {
  var e = cachedContacts[i];
  ...
  setText("contact-detail-position", e.Position);
  setText("contact-detail-phone", e.Phone);
  document.getElementById("contact-detail-call").href = `tel:${e.Phone}`;
};
window.closeContactDetail = () => document.getElementById("modal-contact-detail").classList.add("hidden");
window.changeHistoryPage = (dir) => {
  var max = Math.ceil(allHistoryData.length / HISTORY_PAGE_SIZE) - 1;
  currentHistoryPage = Math.max(0, Math.min(max, currentHistoryPage - dir));
  renderActivityHistory();
};
...
function toggleGlobalNav(s) {
  document.getElementById("global-nav")?.classList.toggle("hidden", !s);
}
function getShortNameClient(n) {
  return n?.split(" ").pop() || "...";
}
function getAvatarHtml(n, u, s, t) {
  return `<img src="${u || "https://ui-avatars.com/api/?name=" + n}" class="${s} rounded-2xl object-cover shadow-sm bg-slate-200">`;
}
function toggleHomeState(s) {
  document.getElementById("state-loading").classList.add("hidden");
  document.getElementById("state-idle").classList.toggle("hidden", s !== "idle");
  document.getElementById("state-working").classList.toggle("hidden", s !== "working");
}
...
function selectReqType(t) {
  currentReqType = t;
  setText("req-type-label", t === "Giải trình" ? "Giải trình công" : t);
  document.getElementById("req-type-menu").classList.add("hidden");
}
function toggleReqDropdown() {
  document.getElementById("req-type-menu").classList.toggle("hidden");
}
