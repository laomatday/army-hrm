...
async function callBackend(functionName, params = []) {
  try {
    ...
    // Xử lý trường hợp response rỗng hoặc không phải JSON
...
document.addEventListener("DOMContentLoaded", function () {
  // --- CẬP NHẬT MỚI: Khôi phục trạng thái chấm đỏ ngay lập tức ---
...
// ==========================================
// 3. TỐI ƯU HÓA: LOGIC LOAD DATA MỚI (SUPER API)
// ==========================================
...
function showMainApp() {
  ...
  // --- BỔ SUNG: TỰ ĐỘNG CẬP NHẬT MỖI 30 GIÂY ---
...
// ==========================================
// HÀM TỔNG QUẢN: LOAD TOÀN BỘ DỮ LIỆU
// ==========================================
async function loadDashboardData() {
  if (!currentUser) return;

  // Gọi Super API từ code.js
  ...
  if (res.success) {
    ...
    // 7. CẬP NHẬT TRẠNG THÁI CHECK-IN (Nút bấm)
    updateCurrentStatusUI();
  } else {
    showToast("error", "Lỗi dữ liệu: " + res.message);
  }
  
  // Tắt loading nếu đang hiện
  toggleHomeState("idle"); 
  // Lưu ý: toggleHomeState có thể bị override bởi updateCurrentStatusUI ở trên, 
  // nên cần đảm bảo logic check-in state được ưu tiên.
  // Trong updateCurrentStatusUI đã gọi toggleHomeState rồi nên dòng trên chỉ là fallback.
}

// --- CÁC HÀM HELPER ĐỂ RENDER UI ---
...
function updateCurrentStatusUI() {
  ...
  // Kiểm tra trong historyData xem hôm nay có record nào chưa checkout không
...
window.switchTab = function (tabName) {
  ...
  // Khi vào tab Requests thì mặc định chuyển sang view Lịch sử
...
function renderNotificationContent(data, mode) {
  ...
  if (hasApprovals) {
    ...
    html +=
      '<div class="text-center py-20 opacity-50 animate-slide-up"><i class="fa-solid fa-check-circle text-4xl mb-3 text-emerald-200"></i><p class="text-xs text-slate-400 font-bold uppercase">Đã duyệt hết các đơn!</p></div>';
  }
  ...
  if (mode !== "approve" && hasMyRequests) {
    ...
    data.myRequests.forEach(function (req) {
      ...
      var unreadIndicator = isUnread
        ? `<span class="absolute top-4 right-4 w-2 h-2 bg-red-500 rounded-full border border-white"></span>`
        : "";
